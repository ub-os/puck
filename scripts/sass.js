...
log.info(`Source files: ${fileNames.map(name => srcPath + name).join(', ')}`)
...
function renderFile(fileName) {
	const sassFile = `${srcPath + fileName}.sass`
	const cssFile = `${distPath + fileName}.css`
	const result = sass.compile(sassFile, {
		importers: [globImporter(srcPath, globImporterTransformer)],
		loadPaths: [srcPath, 'node_modules/'],
		quietDeps: true,
		sourceMap: true,
	})
