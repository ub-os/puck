...
class DebugViewHelper extends AbstractViewHelper
{
    ...
    public function compile(
        ...
        if (!isset($this->arguments['compile']) || !$this->arguments['compile']) {
            $compiler->disable();
        }
        ...
        if (!$arguments['silent']) {
            ...
            if (ApplicationType::fromRequest($GLOBALS['TYPO3_REQUEST'])->isFrontend()) {
                ...
                $pageRenderer->addJsFooterInlineCode(
                    ...
                    sprintf(
                        '%sconsole.%s(%s);',
                        !empty($title) ? 'console.log(\'%c' . addslashes($title) . '\', "color:green");' . PHP_EOL : '',
                        $arguments['level'],
                        $json
                    )
                );
    ...
    protected static function convertAnything($anything, int $depth, array &$convertedIds = []): mixed
    {
        if ($depth < 0) {
        ...
        } elseif ($anything instanceof ObjectStorage || $anything instanceof LazyObjectStorage) {
            $representation = static::convertArray($anything->toArray(), $depth, $convertedIds);
        } else {
    ...
    protected static function convertObject($object, int $depth, array &$convertedIds = []): array
    {
        ...
        foreach ($objectReflection->getProperties(\ReflectionProperty::IS_PUBLIC) as $publicProperty) {
            $propertyName = $publicProperty->getName();
            if (isset(static::$blacklistedProperties[$propertyName]) || isset(static::$blacklistedProperties[get_class($object)][$propertyName])) {
                $gettables[$propertyName] = '(property blacklisted, not called)';
                continue;
            }
            $gettables[$propertyName] = $publicProperty->getValue($object);
        }
