...
class ObserverCollector {
	...
	objectIdentifierString(obj) {
		...
		sortedKeys.forEach((key, i) => {
			const val = obj[key]
			arr.push(`${JSON.stringify(key)}:${JSON.stringify(val)}`)
		})
		return `{${arr.join(',')}}`
	}
	...
	constructor() {
		if (ObserverCollector.#instance) {
			Logger.console.warn(
				'ObserverCollector is a singleton. Use "ObserverCollector.inst()" instead of creating a new instance with "new ObserverCollector()".',
			)
		}
	}
	...
	#singleManager(obsName, defaultOpts) {
		return {
			...
			disconnectAll: () => {
				for (const obsId in this.#observers[obsName]) {
